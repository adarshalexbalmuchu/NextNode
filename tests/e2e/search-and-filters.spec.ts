    ...
    test('should filter by author', async ({ page }) => {
      ...
      if (await advancedSearchButton.isVisible()) {
        ...
        if (await firstResult.isVisible()) {
          await expect(firstResult.locator('[data-testid="post-author"]'))
            .toContainText(TestFilters.AUTHORS[0]);
        }
      }
    });
    ...
    test('should filter by tags', async ({ page }) => {
      ...
      if (await advancedSearchButton.isVisible()) {
        ...
        if (await tagCheckbox.isVisible()) {
          ...
          if (await firstResult.isVisible()) {
            await expect(firstResult.locator('[data-testid="post-tags"]'))
              .toContainText(TestFilters.TAGS[0]);
          }
        }
      }
    });
